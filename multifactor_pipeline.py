...
def _weights_from_holdings(holdings, universe) -> pd.Series:
    """Equal-weight the provided holdings over the full universe index."""
    w = pd.Series(0.0, index=list(universe))
    if holdings:
        w.loc[list(holdings)] = 1.0 / len(holdings)
    return w
...
def build_artifacts():
    ...
    # Benchmark SPY — ensure Series + name (avoids rename errors)
...
def make_one_pager():
    ...
    with PdfPages(out_pdf) as pdf:
        ...
        ax1.imshow(plt.imread(cum_png)) if cum_png.exists() else ax1.text(0.5,0.5,"Missing chart_cumulative.png",
                                                                           ha="center", va="center")

        ax2 = plt.axes([0.52, 0.22, 0.43, 0.30]); ax2.axis("off")
        ax2.imshow(plt.imread(roll_png)) if roll_png.exists() else ax2.text(0.5,0.5,"Missing chart_rolling_alpha.png",
                                                                            ha="center", va="center")

        ax3 = plt.axes([0.05, 0.08, 0.43, 0.28]); ax3.axis("off")
        ax3.imshow(plt.imread(betas_png)) if betas_png.exists() else ax3.text(0.5,0.5,"Missing ff5_mom_betas.png",
                                                                              ha="center", va="center")
